...
from mindsdb.quoting import quote_identifier, safe_connector_name
from mindsdb.transport import MindsDBGateway


class _RecordingMindsDB(MindsDBGateway):
...
def test_identifiers_are_validated_or_escaped_at_the_right_boundary() -> None:
    assert safe_connector_name("orders-prod") == "orders-prod"
    assert quote_identifier("order`items", "table name") == "`order``items`"
    try:
        safe_connector_name("orders; DROP DATABASE prod")
    except ValueError:
        pass
    else:
...
async def test_table_schema_query_escapes_real_database_identifiers() -> None:
    ...
    assert service.queries == ["SHOW COLUMNS FROM `sales-db`.`order``items`"]
...
async def test_query_timeout_is_returned_as_explicit_error() -> None:
    service = MindsDBGateway("http://timeout.test", timeout=0.1)
    with patch("mindsdb.transport.httpx.AsyncClient", return_value=_TimeoutClient()):
